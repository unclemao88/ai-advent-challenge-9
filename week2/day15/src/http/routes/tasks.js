 ...
 * POST   /api/tasks/:id/continue     run the next state (validated transition), or retry a failed one
 ...
 * POST   /api/tasks/:id/auto         switch to auto mode (a task waiting for "continue" runs on)
 * POST   /api/tasks/:id/manual       switch to manual mode (a running auto chain stops after its step)
 * POST   /api/tasks/:id/resolve      after an invariant conflict: {decision: keep | disable | updated | cancel}
 * POST   /api/tasks/:id/cancel       stop the task (state: cancelled)
...
export function createTasksRouter({ agent, tasks, memory, llmLimiter }) {
  ...
  const action = (name, fn, { limited = false } = {}) => {
    router.post(`/tasks/:id/${name}`, ...(limited ? [llmLimiter] : []), asyncRoute(async (req, res) => {
      const id = requireTaskId(req.params.id);
      // These actions take no body; one with fields is a client mistake, not something to ignore.
      if (req.body !== undefined && name !== 'resolve') rejectUnknownFields(requireObject(req.body), []);
  ...
  action('auto', (id, req) => agent.setMode(id, 'auto', { requestId: req.id }), { limited: true });
  action('manual', (id, req) => agent.setMode(id, 'manual', { requestId: req.id }));
