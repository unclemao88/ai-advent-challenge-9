...
const task = { id: 'task-11111111-1111-4111-8111-111111111111', mode: 'manual', objective: 'Ship the service', validation: { attempts: 0 } };
const base = {
  profile: null, invariants: [], shortTerm: [], work: null, longTerm: null, task, state: 'planning', userMessage: 'hi',
};

test('the context builder includes every component, in a fixed order', () => {
  const builder = new ContextBuilder({ tokenCounter: estimateCounter() });
  const ctx = builder.build({
    ...base,
    profile: { style: 'terse', format: 'markdown', limitations: 'none' },
    invariants: [{ id: 'stack', name: 'Stack', value: 'Node.js', category: 'stackLimitations', enabled: true, forbidden: [] }],
  ...
  assert.deepEqual(ctx.order, ['system', 'profile', 'invariants', 'task', 'work', 'longTerm', 'shortTerm', 'taskState', 'request']);

  const system = ctx.messages[0].content;
  assert.match(system, /\[USER PROFILE\]\nStyle: terse\nFormat: markdown\nLimitations: none/);
  assert.match(system, /\[AGENT INVARIANTS\]\nStack limitation:\n- \[stack\] Stack: Node\.js/);
  assert.match(system, /\[CURRENT TASK\]\nTask ID: task-1{8}[\s\S]*Objective: Ship the service[\s\S]*Lifecycle: \[planning\] → execution → validation → done/);
  assert.match(system, /Allowed transitions from planning: planning, execution, paused, failed, cancelled\nYou may propose as the next state: execution/);
  assert.match(system, /\[WORK MEMORY\]\nObjective: Ship it\nPlan:\n1\. a/);
  assert.match(system, /\[LONG-TERM MEMORY\]\nSolutions:\n- \(ltm_x1\) Use nginx/);
  assert.match(system, /\[SHORT-TERM MEMORY\]\nThe 2 most recent message/);
});

test('empty components are stated explicitly, never left out', () => {
  const ctx = new ContextBuilder({ tokenCounter: estimateCounter() }).build({ ...base, userMessage: null });
  const system = ctx.messages[0].content;
  assert.match(system, /\[USER PROFILE\]\nNo profile is set/);
  ...
  assert.match(system, /\[LONG-TERM MEMORY\]\nEmpty/);
  assert.match(system, /\[SHORT-TERM MEMORY\]\nEmpty/);
  assert.match(ctx.messages.at(-1).content, /\[CURRENT REQUEST\]\n\(none: the user pressed "continue"\)/);
});
...
test('the current context count includes profile, invariants, memories, task state and request', () => {
  ...
  const withInvariants = total({ invariants: [{ id: 'a', name: 'Backend', value: 'Node.js and Express only', category: 'stackLimitations', enabled: true, forbidden: [] }] });
  ...
  const withWork = total({ work: { objective: 'Deploy the service on Debian 12 with systemd, nginx and a dedicated user', plan: ['Write the unit file'], requirements: [], decisions: [], facts: [], intermediateResults: [], validationResults: [], variables: {} } });
  const withLong = total({ longTerm: { solutions: [{ id: 'ltm_a', content: 'Use nginx as a reverse proxy in front of port 3015', tags: [] }], knowledge: [] } });
  const withState = total({ notes: { keepInvariants: true } });
  assert.notEqual(total({ state: 'validation' }), empty, 'the task state is part of the count');
  const withTask = total({ task: { ...task, objective: 'A much longer objective that describes the task in considerably more detail' } });
  const withRequest = total({ userMessage: 'hi and a much longer question about everything under the sun' });

  for (const [name, value] of Object.entries({ withProfile, withInvariants, withShort, withWork, withLong, withState, withTask, withRequest })) {
    assert.ok(value > empty, `${name} (${value}) > empty (${empty})`);
  }
});

test('the current request count is computed from exactly the messages that are sent', async (t) => {
  const { agent, llm, profiles, invariants } = await buildApp(t, { llm: fakeLlm() });
  await profiles.updateProfile({ style: 'terse' });
  await invariants.create({ name: 'Stack', value: 'Node.js', category: 'stackLimitations' });
  ...
  const parts = ['system', 'profile', 'invariants', 'task', 'work', 'longTerm', 'shortTerm', 'taskState', 'request'];
  assert.ok(parts.every((k) => breakdown[k] > 0), 'every component is counted');
  assert.ok(breakdown.total >= parts.reduce((n, k) => n + breakdown[k], 0) - 20, 'the total covers every component (± template markers)');
...
test('individual memory layer counts follow what each layer holds', async (t) => {
  const { agent, memory } = await buildApp(t);
  const empty = await agent.tokenSummary();
  assert.deepEqual([empty.shortTerm, empty.work, empty.longTerm], [0, 0, 0]);

  await memory.addMessage({ role: 'user', content: 'a question about systemd units and journald' });
  const withShort = await agent.tokenSummary();
  assert.ok(withShort.shortTerm > 0);
  assert.deepEqual([withShort.work, withShort.longTerm], [0, 0], 'the other layers are unaffected');

  await memory.addMemory('longTerm', { category: 'knowledge', content: 'The server runs Debian 12 with Node.js 22' });
  const withLong = await agent.tokenSummary();
  assert.ok(withLong.longTerm > 0);
  assert.equal(withLong.shortTerm, withShort.shortTerm);
  assert.ok(withLong.currentContext > withLong.shortTerm + withLong.longTerm, 'the context count is more than the layers');
});
