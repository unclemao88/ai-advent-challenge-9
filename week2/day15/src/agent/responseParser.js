 ...
 *
 * `proposedNext` is passed on as the model wrote it (normalised, clipped):
 * the state machine decides whether it is a legal transition, and records it
 * when it is not.
 *
...
export function parseAgentReply(content, state) {
  ...
  return {
    structured: true,
    response,
    proposedNext: typeof json.nextState === 'string' && json.nextState.trim()
      ? json.nextState.trim().toLowerCase().replace(/[^a-z_ -]/g, '').slice(0, 40) || null
      : null,
