...
test('work memory: save, load, merge and log every update', async (t) => {
  ...
  await m.updateWorkMemory(TASK, { decisions: ['use Express', 'port 3014'], variables: { port: 3014 } }, { state: 'execution' });
  ...
  assert.deepEqual(doc.decisions, ['use Express', 'port 3014'], 'lists append without duplicates');
  assert.equal(doc.variables.port, '3014');
  assert.equal(doc.intermediateResults[0].state, 'planning');
  assert.deepEqual(doc.log.map((l) => l.state), ['idle', 'planning', 'execution']);
...
test('the three layers stay in three separate files', async (t) => {
  const dir = await tempDir(t);
  const m = await memoryManager(t, { dataDir: dir });
  ...
  const short = JSON.stringify(await readJson(dir, 'short-term-memory.json'));
  const work = JSON.stringify(await readJson(dir, 'work-memory.json'));
  const long = JSON.stringify(await readJson(dir, 'long-term-memory.json'));
...
test('switching a layer to another provider copies its data and leaves the others alone', async (t) => {
  ...
  const file = JSON.stringify(await readJson(dir, 'long-term-memory.json'));
  assert.ok(!file.includes('volatile only'), 'the memory provider writes nothing to disk');

  const config = await readJson(dir, 'storage-config.json');
...
test('a corrupt layer file is moved aside, never overwritten', async (t) => {
  const dir = await tempDir(t);
  await writeFile(path.join(dir, 'work-memory.json'), '{ not json');
  const m = await memoryManager(t, { dataDir: dir });
...
test('retrieval sends only relevant long-term entries', () => {
  const now = new Date();
  const items = [
    { id: 'ltm_a1', category: 'solutions', content: 'Nginx reverse proxy config for port 3014', tags: ['deploy'], updatedAt: now.toISOString() },
...
test('memory commands are explicit and deterministic', () => {
  ...
  assert.deepEqual(work, { requirements: ['port 3014'] });
});
