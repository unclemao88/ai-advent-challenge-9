 ...
 * why something was stored. Long-term memory is written by these rules, the
 * memory panel, promotions and approved suggestions — never by the
 * conversation itself.
 *
...
const LONG_TERM_RULES = [
  { pattern: /^remember\s+(?:solution|fix|procedure|approach)s?\s*:\s*(.+)$/i, category: 'solutions' },
  { pattern: /^remember(?:\s+(?:knowledge|fact|this|that))?\s*:\s*(.+)$/i, category: 'knowledge' },
];
