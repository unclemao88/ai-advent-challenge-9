...
export const MEMORY_LAYERS = Object.freeze({
  shortTerm: { label: 'Short-term memory', file: 'short-term-memory.json', purpose: 'The current conversation.' },
  work: { label: 'Work memory', file: 'work-memory.json', purpose: 'Objectives, decisions and results of each task.' },
  longTerm: { label: 'Long-term memory', file: 'long-term-memory.json', purpose: 'Solutions and knowledge kept across tasks (the profile lives in data/profile.json).' },
});
...
/**
 * Persists which provider each layer uses, in `data/storage-config.json`:
 *
...
export class StorageConfigStore {
  ...
  constructor({ dataDir, logger, defaults = {} }) {
    ...
    this.doc = new JsonDocument({
      dataDir, file: 'storage-config.json', logger, empty: () => this.defaultConfig(), normalize: (v) => this.#normalize(v),
    });
  }
