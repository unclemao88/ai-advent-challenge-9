...
import { InvalidTransitionError } from '../agent/stateMachine.js';
...
/** A request id for every API call: returned as X-Request-Id and put on every log line of the request. */
export function requestId(req, res, next) {
  const given = req.get('x-request-id');
  req.id = given && /^[A-Za-z0-9._-]{8,64}$/.test(given) ? given : randomUUID();
  res.set('X-Request-Id', req.id);
  next();
}

/** One structured line per API request: id, method, route, status, duration. Never bodies. */
  ...
  return (req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      logger[level]('http.request', {
        requestId: req.id, method: req.method, path: req.originalUrl.split('?')[0], status: res.statusCode, ms: Math.round(ms), ip: req.ip,
      });
    });
    next();
  };
...
export function errorHandler(logger) {
  ...
  return (err, req, res, next) => {
    ...
    if (err instanceof AppError || (err?.name === 'DeepSeekError')) {
      ...
      if (status >= 500) logger.error('http.error', { requestId: req.id, path: req.path, code: err.code, error: err });
      ...
      logger.error('storage.failure', { requestId: req.id, method: req.method, path: req.path, error: err });
      ...
      logger.error('http.unexpected_error', { requestId: req.id, method: req.method, path: req.path, error: err });
    }
