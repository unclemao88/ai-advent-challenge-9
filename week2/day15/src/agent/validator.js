...
export class Validator extends StepRunner {
  ...
  async run({ task, invariants, lastResult, userMessage = null, notes = {}, requestId }) {
    const { reply, usage, profileCheck } = await this.call(task, STATES.VALIDATION, { userMessage, notes, requestId });

    const byRule = lastResult ? this.checker.check({ type: 'validation', text: lastResult }, invariants).conflicts : [];
    const byModel = this.checker.fromModel(reply.invariantConflicts, invariants, 'validation');
