 ...
 * `data/work-memory.json` with the JSON provider. It lives as long as the task
...
export const LIMITS = Object.freeze({
  ...
  variables: 30, variableName: 60, variableValue: 500, log: 50, checks: 20,
});
...
export class WorkMemory {
  ...
  createWorkMemory(taskId, { objective = '' } = {}) {
    return this.#queue.run(async () => {
      ...
      addLog(doc, { source: 'task', state: 'idle', fields: ['objective'] });
 ...
 * in it. The log and the invariant checks are bookkeeping and stay out; only
