...
export function loadConfig(env = process.env) {
  ...
  return {
    nodeEnv,
    port: int(env.PORT, { min: 0, max: 65535 }) ?? 3014,
    ...
    agent: {
      ...
      maxInvariantRevisions: int(env.MAX_INVARIANT_REVISIONS, { min: 0, max: 5 }) ?? 1,
      longTermContextTokens: int(env.LONG_TERM_CONTEXT_TOKENS, { min: 0, max: 100_000 }) ?? 2000,
      maxContextTokens: int(env.MAX_CONTEXT_TOKENS, { min: 2000, max: 1_000_000 }) ?? 100_000,
    },
    memory: {
      shortTermMaxMessages: int(env.SHORT_TERM_MAX_MESSAGES, { min: 2, max: 500 }) ?? 30,
      // Initial storage provider per layer; the UI can change it later (data/storage-config.json).
      providers: {
        shortTerm: env.STORAGE_SHORT_TERM || 'json',
        work: env.STORAGE_WORK || 'json',
        longTerm: env.STORAGE_LONG_TERM || 'json',
      },
    },
    security: {
