 ...
 *                total_tokens). Used for API input/output/total, for the token
 *                count of each stored answer, and for the stored summary.
 *   ESTIMATED  — count() below. DeepSeek does not publish a JavaScript
 *                tokenizer, so anything we count ourselves (a question, a
 *                fallback when usage is missing) is an estimate, is stored with
...
(function (root, factory) {
  ...
  /**
   * The token count of the stored summary record ({summary, tokens, tokensSource}).
   * @returns {{tokens: number, estimated: boolean}}
   */
  function countSummary(record) {
    if (!record || typeof record.summary !== 'string' || !record.summary) {
      return { tokens: 0, estimated: false };
    }
    var stored = Number(record.tokens);
    if (typeof record.tokens === 'number' && isFinite(stored) && stored >= 0) {
      return { tokens: Math.round(stored), estimated: record.tokensSource !== 'api' };
    }
    return { tokens: count(record.summary), estimated: true };
  }
