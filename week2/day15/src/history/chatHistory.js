...
/**
 * The chat history, `data/history/chat-history.json`:
 *
 *   { "messages": [ { "id", "role", "tag": "you asked" | "agent answered",
 *                     "content", "date": "2026-09-19", "time": "12:15:03",
 *                     "timestamp": "2026-09-19T12:15:03.123Z",
 *                     "taskId", "kind", "task", … } ] }
 *
...
export class ChatHistory {
  constructor({ dataDir, logger }) {
    ...
    this.doc = new JsonDocument({
      dataDir, file: DATA_FILES.history, logger, empty: () => ({ messages: [] }), normalize,
    });
  }
  ...
  clear() {
    return this.doc.update(() => {
      this.logger.info('history.clear');
...
function normalize(value) {
  ...
  return {
    messages: messages.filter((m) => isPlainObject(m) && TAGS[m.role] && typeof m.content === 'string'
      ...
      .map((m) => ({ ...m, tag: TAGS[m.role], ...(m.date && m.time ? {} : dateParts(new Date(m.timestamp))) })),
  };
}

/**
 * Date and time as stored with each message (UTC, like the timestamp). The
 * browser shows them in the viewer's local time zone from `timestamp`.
 */
export function dateParts(date) {
  const iso = Number.isNaN(date.getTime()) ? new Date(0).toISOString() : date.toISOString();
