...
import { mergeSettings } from '../settings/settingsStore.js';
import { HttpError } from '../utils/httpError.js';
import { SerialQueue } from '../utils/serialQueue.js';
import { memoryOverview } from './memory.js';

/**
 * GET  /api/settings  current memory settings and the available storage modes
 * POST /api/settings  { memory: { <layer>: { storage, maxMessages? } } }
 */
export function createSettingsRouter({ agent, memory, settingsStore }) {
  ...
  router.get('/settings', (req, res) => {
    res.json({ settings: settingsStore.get(), storageModes: listStorageModes() });
  });
