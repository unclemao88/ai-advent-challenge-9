import { ACTIVE_STATES, STATES } from '../state-machine/StateMachine.js';
import { isCategory } from '../memory/LongTermMemory.js';
import { clip, isPlainObject } from '../utils/validate.js';

const SUGGESTABLE = new Set([...ACTIVE_STATES, STATES.DONE]);
 ...
 * JSON contract was met, which is logged.
 *
...
export function parseAgentReply(content, state) {
  ...
  if (!json) {
    return {
      structured: false,
      response: clip(raw, MAX_RESPONSE_CHARS),
      suggestedNext: null,
      plannedAction: null,
  ...
  const response = typeof json.response === 'string' && json.response.trim()
    ...
    // Valid JSON without a usable response: show something readable rather than raw JSON.
    : clip(typeof json.validation?.summary === 'string' ? json.validation.summary : 'The agent returned no text for this step.', MAX_RESPONSE_CHARS);

  let validation = null;
  if (state === STATES.VALIDATION) {
    validation = isPlainObject(json.validation) && typeof json.validation.passed === 'boolean'
      ? { passed: json.validation.passed, summary: clip(json.validation.summary, 1000) }
...
function inferVerdict(text) {
  if (/\b(validation|check)\s+(failed|did not pass)\b|\bFAILED\b/.test(text)) return { passed: false, summary: '' };
  if (/\b(validation|check)\s+passed\b|\bPASSED\b/.test(text)) return { passed: true, summary: '' };
  return null;
}
