/*
 * DeepSeek Agent (day 14) — browser client.
 *
...
(function () {
  ...
  function stateBlock(task, conflict) {
    ...
    return h('div', { class: `state-block${conflict ? ' state-block--conflict' : ''}` },
      h('span', null, 'Current state: ', h('b', null, task.state)),
    ...
    return h('div', { class: 'proposals' }, items.map((p) => {
      ...
      const row = h('div', { class: 'proposal' },
        h('span', { class: 'proposal__text' },
          h('span', { class: 'proposal__cat' }, `Suggested for long-term memory · ${p.category}`), h('br'), p.content));
      if (saved) {
  ...
  function renderMessage(m) {
    if (m.kind === 'status' || m.kind === 'error') {
      return h('div', { class: `note${m.kind === 'error' ? ' note--error' : ''}`, dataset: { id: m.id } },
        h('time', { datetime: m.timestamp }, fmtDateTime(m.timestamp)), m.content);
    }
    ...
    if (!isUser) {
      ...
      if (isConflict && isOpenConflictMessage(m)) bubble.append(conflictButtons(m.conflicts));
  ...
  /** Only the newest conflict message of the active task, while its conflict is open, gets buttons. */
  function isOpenConflictMessage(m) {
    const t = state.task;
    if (!t || !t.pendingConflict || m.taskId !== t.id) return false;
    const last = state.messages.filter((x) => x.kind === 'conflict' && x.taskId === t.id).at(-1);
    return last?.id === m.id;
  }
  ...
  const STEPS = ['idle', 'planning', 'execution', 'validation', 'done'];
  ...
  function renderTask() {
    ...
    $('#task-next').textContent = t ? (t.nextState || '—') : '—';
    ...
    if (t) {
      ...
      else if (t.status === 'paused') { label = 'paused'; cls = 'pill--warn'; }
      else if (t.status === 'done') { label = 'done'; cls = 'pill--ok'; }
      else if (t.status === 'failed') { label = t.resumeState ? 'failed · retry possible' : 'cancelled'; cls = 'pill--danger'; }
      else if (t.state === 'waiting_for_user') { label = t.waitingReason === 'invariant_conflict' ? 'invariant conflict' : 'waiting for you'; cls = 'pill--warn'; }
      else { label = 'waiting for continue'; }
      if (t.pauseRequested) label += ' · pause requested';
    }
    status.textContent = label;
    status.className = `pill ${cls}`;

    // Stepper: past / current, with waiting_for_user and paused shown on the step they belong to.
    const shown = t ? (t.state === 'paused' ? t.resumeState : t.state) : null;
    const at = t?.state === 'waiting_for_user' || t?.state === 'failed' ? STEPS.indexOf(t.nextState ?? 'planning') : STEPS.indexOf(shown);
    ...
    const mode = t && t.status !== 'done' && !(t.status === 'failed' && !t.resumeState) ? t.mode : state.selectedMode;
    $$('.segmented button').forEach((b) => {
      b.setAttribute('aria-checked', String(b.dataset.mode === mode));
      b.disabled = running;
    });

    const open = t && (t.status === 'active' || t.status === 'paused' || (t.status === 'failed' && t.resumeState));
    const canContinue = t && !running && t.nextState && (t.status === 'active' || (t.status === 'failed' && t.resumeState))
      && !(t.state === 'waiting_for_user' && t.waitingReason === 'invariant_conflict');
    $('#btn-continue').disabled = !canContinue;
    $('#btn-continue').textContent = t?.status === 'failed' ? 'Retry' : (t?.nextState === 'done' ? 'Finish' : 'Continue');
    $('#btn-pause').hidden = t?.status === 'paused';
    $('#btn-pause').disabled = !t || t.status !== 'active' || t.state === 'idle' || t.pauseRequested;
    $('#btn-resume').hidden = t?.status !== 'paused';
    $('#btn-resume').disabled = running;
    $('#btn-new-task').disabled = !t || running;
    $('#btn-cancel').disabled = !open || running;

    const errEl = $('#task-error');
    errEl.hidden = !(t?.status === 'failed' && t.lastError);
    errEl.textContent = t?.lastError ? `Last error: ${t.lastError}` : '';

    const box = $('#conflict-box');
    if (t?.pendingConflict && t.state === 'waiting_for_user') {
    ...
    if (t && t.status === 'paused') hint = 'The task is paused: resume it, or press "New task" first.';
    else if (t && running) hint = 'The agent is working on this task…';
    else if (t && t.state === 'waiting_for_user') hint = 'Your message answers the agent and continues the task.';
    else if (t && t.status === 'active') hint = `Your message is input for the ${t.state === 'validation' ? 'execution' : t.state} step of this task.`;
    else if (t && t.status === 'failed' && t.resumeState) hint = `Your message retries the ${t.resumeState} step.`;
  ...
  function applyTokens(tokens) {
    ...
    if (b) {
      $('#tok-breakdown').textContent = `context = system ${fmtNum(b.system)} · profile ${fmtNum(b.profile)} · invariants ${fmtNum(b.invariants)}`
        + ` · short-term ${fmtNum(b.shortTerm)} · work ${fmtNum(b.work)} · long-term (relevant) ${fmtNum(b.longTerm)}`
  ...
  function schedulePreview() {
    ...
    state.previewTimer = setTimeout(async () => {
      ...
      try {
        const data = await api('POST', '/token-count', { message: $('#question').value });
  ...
  function updateAskButton() {
    const t = state.task;
    const blocked = state.busy || t?.status === 'paused' || t?.stepStatus === 'running';
  ...
  $('#ask-form').addEventListener('submit', async (event) => {
    ...
    renderChat();
    const mode = state.task && state.task.status !== 'done' ? undefined : state.selectedMode;
  ...
  $$('.segmented button').forEach((button) => button.addEventListener('click', async () => {
    ...
    storageSet('deepseek-agent-mode', mode);
    const t = state.task;
    if (t && t.status !== 'done' && !(t.status === 'failed' && !t.resumeState)) {
      try {
        applyTask((await api('POST', `/tasks/${t.id}/mode`, { mode })).task);
      } catch (err) {
        showError(err);
      }
    } else {
  ...
  function renderInvariantsModal(data) {
    ...
    if (!data.invariants.length) {
      ...
      list.replaceChildren(...data.invariants.map((inv) => h('div', { class: `inv-card${inv.enabled ? '' : ' is-off'}` },
        h('div', null,
          ...
          h('div', { class: 'inv-card__meta' }, `${inv.id} · ${inv.category} · ${inv.enabled ? 'enabled' : 'disabled'}`
    ...
    if (!select.options.length) data.categories.forEach((c) => select.append(h('option', { value: c }, c)));
  }
  ...
  async function tabShort() {
    ...
    return [
      ...
      h('p', { class: 'muted' }, 'The recent conversation the agent remembers and replays to DeepSeek. Clearing it does not delete the visible chat history (data/conversation.json).'),
  ...
  async function tabStorage() {
    ...
    return [
      ...
      h('p', { class: 'muted' }, `Configuration file: ${data.location}. Profile, invariants, tasks and the conversation log are always JSON files.`),
    ];
  }
  ...
  async function tabTasks() {
    ...
    return h('div', { class: 'table-wrap' }, h('table', { class: 'data' },
      ...
      h('tbody', null, data.tasks.map((t) => h('tr', null,
        ...
        h('td', null, `${t.state} (${t.status})`),
        h('td', null, t.nextState || '—'),
        h('td', null, t.mode),
  ...
  async function tabContext() {
    const data = await api('POST', '/token-count', { message: input.value, includeText: true });
    applyTokens(data.tokens);
    const b = data.tokens.breakdown;
    const order = [['system', 'System instructions'], ['profile', 'User profile'], ['invariants', 'Active invariants'], ['shortTerm', 'Short-term memory'],
      ['work', 'Work memory'], ['longTerm', 'Relevant long-term memory'], ['taskState', 'Task state'], ['request', 'Current request']];
    return [
  ...
  async function init() {
    ...
    try {
      ...
      banner.textContent = 'DEEPSEEK_API_KEY is not configured on the server. The UI works, but questions will fail until it is set in .env.';
    } catch (err) {
    ...
    try {
      const [conv, profile] = await Promise.all([api('GET', '/conversation'), api('GET', '/profile'), loadInvariants(), refreshLongTerm()]);
      ...
      if (conv.busy) {
        ...
        const wait = setInterval(async () => {
          ...
          if (d && !d.busy) {
            clearInterval(wait);
            setBusy(false);
            const fresh = await api('GET', '/conversation');
