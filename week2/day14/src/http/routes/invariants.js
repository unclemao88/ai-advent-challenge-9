...
import { INVARIANT_CATEGORIES, formatInvariants } from '../../invariants/invariantManager.js';
...
/**
 * GET    /api/invariants             every invariant + how the active ones are sent to DeepSeek
...
export function createInvariantsRouter({ invariants, agent, tokenCounter }) {
  ...
  router.post('/invariants/check', asyncRoute(async (req, res) => {
    ...
    const type = requireEnum(body.type ?? 'request', 'type', ['request', 'plan', 'response']);
