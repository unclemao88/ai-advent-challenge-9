...
import { ACTIVE_STATES, ALL_STATES, MODES, STATE_ACTIONS, TRANSITIONS } from '../../state-machine/StateMachine.js';
...
export function createConfigRouter({ config, llm, tokenCounter, memory, agent }) {
  const router = Router();
  router.get('/config', (req, res) => {
    res.json({
      app: { name: 'DeepSeek Agent', version: '1.0.0', port: config.port },
      llm: {
      ...
      agent: agent.options,
      stateMachine: { states: ALL_STATES, activeStates: ACTIVE_STATES, transitions: TRANSITIONS, actions: STATE_ACTIONS, modes: MODES },
