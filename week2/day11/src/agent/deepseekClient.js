 ...
 *
 *   const { content, usage } = await client.send(messages);
 *
...
export class DeepSeekClient {
   ...
   *          fetchImpl?: typeof fetch}} [options] `fetchImpl` exists for tests.
   */
  ...
  async send(messages) {
    ...
    const response = await this.#post({ model: this.model, messages, stream: false });
